                    ...
                    <div className="glass max-w-md w-full p-6 rounded-2xl border border-zinc-800 shadow-2xl animate-in fade-in zoom-in-95 duration-200 bg-zinc-950/90">
                        ...
                        <div className="space-y-4 text-xs leading-relaxed text-zinc-300">
                            ...
                            <ol className="list-decimal list-inside space-y-3 font-medium">
                                ...
                                <li>
                                    {t('header.manualStep3Before')} <span className="text-white font-bold inline-flex items-center gap-1 px-1.5 py-0.5 bg-zinc-800 rounded border border-zinc-700 shadow-sm mx-1"><FolderOpen size={10} /> {t('header.manualStep3Browse')}</span> {t('header.manualStep3After')}
                                </li>
                                <li>{t('header.manualStep4Before')} <code className="text-primary font-bold">{t('header.manualStep4Executable')}</code> {t('header.manualStep4After')}</li>
                            </ol>
            ...
            <div className="flex-1 flex justify-start items-center gap-5">
                <div className="flex items-center gap-3 group/header">
                    <div className="flex items-center gap-1.5 grayscale opacity-50 group-hover/header:grayscale-0 group-hover/header:opacity-100 transition-all">
                        ...
                        <span className="text-[9px] uppercase font-black text-zinc-500 tracking-tighter">{t('header.themeLabel')}</span>
                    </div>
                    <div className="flex items-center gap-2">
                        {[
                            { id: 'ultraviolet', color: '#8b5cf6', label: t('header.themes.ultraviolet') },
                            { id: 'astro', color: '#3b82f6', label: t('header.themes.astro') },
                            { id: 'carbon', color: '#ffffff', label: t('header.themes.carbon') },
                            { id: 'emerald', color: '#10b981', label: t('header.themes.emerald') },
                            { id: 'bloodmoon', color: '#ef4444', label: t('header.themes.bloodmoon') }
                        ].map((th) => (
                            <button
                                key={th.id}
                ...
                <div className="relative" ref={langMenuRef}>
                    ...
                    >
                        <Languages size={12} />
                        <span className="text-[9px] font-black uppercase tracking-widest">{translations.languages[locale]}</span>
            ...
            <div className="flex flex-wrap gap-3 items-center flex-1 justify-end">
                {!binaryStatus.found && (
                    ...
                    >
                        ...
                        <span className="text-[10px] font-black uppercase tracking-widest">{t('header.setupHelp')}</span>
                    </button>
                )}
                ...
                <div className="glass px-4 py-2 rounded-xl flex items-center gap-4 w-full md:w-auto justify-between md:justify-start border border-zinc-800 bg-zinc-950/50 backdrop-blur-2xl shadow-2xl relative group overflow-hidden">
                    ...
                    <div className="flex flex-col min-w-[110px] relative z-10">
                        <div className="flex items-center gap-1.5 mb-1 text-zinc-500">
                            <span className="text-[10px] uppercase font-black tracking-widest">{t('header.scrcpyEngine')}</span>
                    ...
                    <div className="flex gap-2 items-center border-l border-zinc-800 pl-3 relative z-10">
                        ...
                        <button onClick={onSetPath} className="p-1 hover:text-primary text-zinc-500 transition-colors" title={t('header.selectFolder')}><FolderOpen size={16} /></button>
                        <button onClick={onResetPath} className="p-1 hover:text-red-400 text-zinc-500 transition-colors" title={t('header.resetPath')}><RefreshCcw size={16} /></button>
                    </div>
                </div>
            </div>
        </header>
